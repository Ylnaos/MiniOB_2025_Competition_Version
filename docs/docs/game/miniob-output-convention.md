...
3. 对于QUERY操作，如果执行失败，返回FAILURE（包括语法错误）。否则按照下面的格式要求输出：列名显示和顺序说明：
    ...
    - 注意：查询成功时**不要**额外输出`SUCCESS`或其它提示语，只保留表头和数据行。
    ...
    ```
    C1 | C2 | C3
    1 | [8.12,9.92,7.21,5.58,1.76,9.02,5.29,0.36,3.44,0.66] | [8.39,3.06,2.54,4.95,6.13,2.13,5.2,6.05,0.53,5.53]
    ```

    如果查询结果为空，也仅输出表头，无需追加`SUCCESS`。
