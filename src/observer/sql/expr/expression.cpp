...
static RC eval_tokenize_value(const Value &text_value, const Value *parser_value, Value &output)
{
  ...
  RC rc = TextTokenizer::tokenize(text_value.get_string(), parser, tokens);
  if (OB_FAIL(rc)) {
    return rc;
  }
...
FullTextMatchExpr::FullTextMatchExpr(std::vector<std::unique_ptr<Expression>> columns,
                                     std::unique_ptr<Expression> query_expr)
    : columns_(std::move(columns)), query_expr_(std::move(query_expr))
{}

unique_ptr<Expression> FullTextMatchExpr::copy() const
{
  std::vector<std::unique_ptr<Expression>> column_copies;
  column_copies.reserve(columns_.size());
  for (const auto &col : columns_) {
    column_copies.emplace_back(col ? col->copy() : nullptr);
  }
  std::unique_ptr<Expression> query_copy = query_expr_ ? query_expr_->copy() : nullptr;
  auto expr = make_unique<FullTextMatchExpr>(std::move(column_copies), std::move(query_copy));
  expr->set_table(table_);
  expr->set_relation_name(relation_name_);
  expr->set_field_names(field_names_);
  expr->set_parser_name(parser_name_);
  expr->set_name(name());
  if (const char *alias_name = alias()) {
    expr->set_alias(alias_name);
  }
  return expr;
}

RC FullTextMatchExpr::ensure_corpus_stats_ready() const
{
  if (stats_ready_) {
    return RC::SUCCESS;
  }
  if (table_ == nullptr) {
    LOG_WARN("full-text match expression missing bound table");
    return RC::INTERNAL;
  }
  ...
  RC rc = const_cast<Table *>(table_)->get_record_scanner(scanner, nullptr, ReadWriteMode::READ_ONLY);
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to initialize record scanner for full-text stats. rc=%s", strrc(rc));
    return rc;
  }
  std::unique_ptr<RecordScanner> scanner_holder(scanner);
  RecordScanner *scanner_ptr = scanner_holder.get();

  RowTuple row_tuple;
  const vector<FieldMeta> *fields = table_->table_meta().field_metas();
  row_tuple.set_schema(table_, fields, relation_name_);

  Record record;
  double total_len = 0.0;

  while (RC::SUCCESS == (rc = scanner_ptr->next(record))) {
    row_tuple.set_record(&record);

    string doc_text;
    rc = gather_document_text(row_tuple, doc_text);
    if (OB_FAIL(rc)) {
      break;
    }

    vector<string> tokens;
    rc = tokenize_document(doc_text, tokens);
    if (OB_FAIL(rc)) {
      break;
    }
    ...
    ++doc_count_;
    total_len += static_cast<double>(tokens.size());
  ...
  if (doc_count_ > 0) {
    avg_doc_len_ = total_len / static_cast<double>(doc_count_);
  } else {
    avg_doc_len_ = 0.0;
  }
  stats_ready_ = true;
...
RC FullTextMatchExpr::gather_document_text(const Tuple &tuple, string &text) const
{
  text.clear();
  bool appended = false;
  Value cell;
  for (const auto &col : columns_) {
    ...
    RC rc = col->get_value(tuple, cell);
    if (OB_FAIL(rc)) {
      return rc;
    }
    ...
    if (cell.attr_type() != AttrType::CHARS && cell.attr_type() != AttrType::TEXTS) {
      continue;
    }
    string fragment = cell.get_string();
    if (fragment.empty()) {
      continue;
    }
    if (appended) {
...
RC FullTextMatchExpr::tokenize_document(const string &text, vector<string> &tokens) const
{
  return TextTokenizer::tokenize(text, parser_name_, tokens);
}

RC FullTextMatchExpr::get_value(const Tuple &tuple, Value &value) const
{
  value.set_float(0.0f);
  RC rc = ensure_corpus_stats_ready();
  if (OB_FAIL(rc)) {
    value.set_null();
    return rc;
  }
  ...
  Value query_raw;
  rc = query_expr_->get_value(tuple, query_raw);
  if (OB_FAIL(rc)) {
    return rc;
  }
  if (query_raw.attr_type() == AttrType::NULLS) {
    value.set_float(0.0f);
    return RC::SUCCESS;
  }
  if (query_raw.attr_type() != AttrType::CHARS && query_raw.attr_type() != AttrType::TEXTS) {
    LOG_WARN("full-text match query must be string, got type=%d", static_cast<int>(query_raw.attr_type()));
    return RC::INVALID_ARGUMENT;
  }

  string query_text = query_raw.get_string();
  vector<string> query_tokens;
  rc = tokenize_document(query_text, query_tokens);
  if (OB_FAIL(rc)) {
    return rc;
  }
  ...
  string doc_text;
  rc = gather_document_text(tuple, doc_text);
  if (OB_FAIL(rc)) {
    return rc;
  }
  vector<string> doc_tokens;
  rc = tokenize_document(doc_text, doc_tokens);
  if (OB_FAIL(rc)) {
    return rc;
  }
  if (doc_tokens.empty()) {
  ...
  unordered_map<string, int> doc_tf;
  for (const string &token : doc_tokens) {
    ++doc_tf[token];
  }

  const double k1 = 1.5;
  const double b  = 0.75;
  const double avg_len = avg_doc_len_ > 0.0 ? avg_doc_len_ : 1.0;
  ...
  for (const auto &entry : query_tf) {
    ...
    int df = df_it == doc_freq_.end() ? 0 : df_it->second;
    if (df <= 0) {
      continue;
    }
    double numerator = static_cast<double>(doc_count_ - df) + 0.5;
    double denominator = static_cast<double>(df) + 0.5;
    if (denominator <= 0.0) {
      continue;
    }
    double idf = std::log(numerator / denominator + 1.0);
    double norm = tf + k1 * (1.0 - b + b * doc_len / avg_len);
    if (norm <= 0.0) {
      continue;
    }
    double tf_weight = (tf * (k1 + 1.0)) / norm;
    score += idf * tf_weight * qf;
  }
...
RC FullTextMatchExpr::try_get_value(Value &value) const
{
  value.set_null();
  return RC::UNIMPLEMENTED;
}
    ...
    const ParsedSqlNode &node, const Tuple &outer_tuple, bool &did_substitute) const
{
  ...
  Session *session = Session::current_session();
  Db      *current_db = session ? session->get_current_db() : nullptr;
  ...
  auto try_substitute_relattr = [&](RelAttrSqlNode &attr, int &is_attr_flag, Value &holder) -> RC {
    if (is_attr_flag != 1) {
      return RC::SUCCESS;
    }
    const char *rel_name   = attr.relation_name.c_str();
    ...
    auto belongs_to_inner = [&](const char *name) -> bool {
      ...
      for (const auto &inner : inner_names) {
        if (0 == strcasecmp(inner.c_str(), name)) {
          return true;
        }
      }
      return false;
    ...
    if (rel_name != nullptr && *rel_name != '\0' && belongs_to_inner(rel_name)) {
      return RC::SUCCESS;
    }

    if ((rel_name == nullptr || *rel_name == '\0') && current_db != nullptr) {
      for (const auto &inner : inner_names) {
        Table *inner_table = current_db->find_table(inner.c_str());
        if (inner_table == nullptr) {
          continue;
        }
        if (inner_table->table_meta().field(field_name) != nullptr) {
          return RC::SUCCESS;
        }
      }
    }
    ...
    if (rel_name != nullptr && *rel_name != '\0') {
      rc = outer_tuple.find_cell(TupleCellSpec(rel_name, field_name), fetched);
      if (rc != RC::SUCCESS && current_db != nullptr) {
        Table *tbl = current_db->find_table(rel_name);
        if (tbl != nullptr) {
          rc = outer_tuple.find_cell(TupleCellSpec(tbl->name(), field_name), fetched);
        }
      }
    }
    if (rc != RC::SUCCESS) {
      rc = outer_tuple.find_cell(TupleCellSpec(field_name), fetched);
    }
    if (rc != RC::SUCCESS) {
      return rc;
    }
  ...
  for (const auto &cond : src.conditions) {
    ...
    rc = try_substitute_relattr(new_cond.left_attr, new_cond.left_is_attr, new_cond.left_value);
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to substitute outer reference in left attr");
      return nullptr;
    }
    rc = try_substitute_relattr(new_cond.right_attr, new_cond.right_is_attr, new_cond.right_value);
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to substitute outer reference in right attr");
      return nullptr;
    }
    dst.conditions.emplace_back(std::move(new_cond));
  }
  ...
  for (const auto &cond : src.having) {
    ...
    rc = try_substitute_relattr(new_cond.left_attr, new_cond.left_is_attr, new_cond.left_value);
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to substitute outer reference in having left attr");
      return nullptr;
    }
    rc = try_substitute_relattr(new_cond.right_attr, new_cond.right_is_attr, new_cond.right_value);
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to substitute outer reference in having right attr");
      return nullptr;
    }
    dst.having.emplace_back(std::move(new_cond));
  }
