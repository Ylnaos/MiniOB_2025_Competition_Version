...
RC MysqlCommunicator::read_event(SessionEvent *&event)
{
  ...
  if (!authed_) {
    ...
    // Record client's capability flags as-is; modern clients (e.g., PyMySQL)
    // often set CLIENT_DEPRECATE_EOF and handle OK at end of resultset.
    // We will respect this and send OK when requested, EOF otherwise.
