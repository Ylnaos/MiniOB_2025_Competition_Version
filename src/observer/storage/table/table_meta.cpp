                   ...
                   StorageEngine storage_engine)
{
  ...
  // add null bitmap for visible fields
  int visible_field_num = static_cast<int>(attributes.size());
  null_bitmap_size_     = (visible_field_num + 7) / 8;
  ...
  for (size_t i = 0; i < attributes.size(); i++) {
    ...
    int vector_dimension = -1;  // 保存原始向量维度
    if (attr_info.type == AttrType::VECTORS) {
      ...
      vector_dimension = field_len;  // 保存原始维度
    ...
    if (vector_dimension >= 0) {
      fields_[i + trx_field_num].set_vector_dimension(vector_dimension);
    }
...
int TableMeta::deserialize(istream &is)
{
  ...
  // reconstruct null bitmap info
  int sys_num = sys_field_num();
  int vis_num = static_cast<int>(fields_.size()) - sys_num;
  null_bitmap_size_   = (vis_num + 7) / 8;
  if (vis_num > 0) {
