...
static bool table_meta_has_record_layout(const TableMeta *table_meta, int record_size)
{
  if (table_meta == nullptr || table_meta->field_num() == 0 || record_size <= 0) {
    return false;
  }

  vector<const FieldMeta *> fields;
  fields.reserve(table_meta->field_num());
  for (int i = 0; i < table_meta->field_num(); ++i) {
    const FieldMeta *field = table_meta->field(i);
    if (field == nullptr || field->offset() < 0 || field->len() <= 0 ||
  ...
  sort(fields.begin(), fields.end(), [](const FieldMeta *lhs, const FieldMeta *rhs) {
  ...
  for (const FieldMeta *field : fields) {
    ...
    end_offset = field->offset() + field->len();
  }
  return end_offset <= record_size;
}

static vector<PaxSegment> build_pax_segments(const TableMeta *table_meta, int record_size)
{
  vector<PaxSegment> segments;
  if (table_meta == nullptr) {
    return segments;
  }

  if (!table_meta_has_record_layout(table_meta, record_size)) {
    segments.reserve(table_meta->field_num());
    for (int i = 0; i < table_meta->field_num(); ++i) {
      const FieldMeta *field = table_meta->field(i);
      segments.push_back(PaxSegment{0, field->len(), field->field_id(), field->visible()});
    }
    return segments;
  }
  ...
  for (const FieldMeta *field : fields) {
    if (field->offset() > next_offset) {
      segments.push_back(PaxSegment{next_offset, field->offset() - next_offset, -1, false});
    }
    segments.push_back(PaxSegment{field->offset(), field->len(), field->field_id(), field->visible()});
    next_offset = field->offset() + field->len();
  }
  if (next_offset < record_size) {
    segments.push_back(PaxSegment{next_offset, record_size - next_offset, -1, false});
  }
  return segments;
}
...
RC RecordPageHandler::init(DiskBufferPool &buffer_pool, LogHandler &log_handler, PageNum page_num, ReadWriteMode mode,
    LobFileHandler *lob_handler, const TableMeta *table_meta)
{
    ...
    int record_size, TableMeta *table_meta, LobFileHandler *lob_handler)
{
  ...
  vector<PaxSegment> pax_segments;
  if (table_meta != nullptr && storage_format_ == StorageFormat::PAX_FORMAT) {
    pax_segments = build_pax_segments(table_meta, record_size);
    column_num   = static_cast<int>(pax_segments.size());
  }
  ...
  for (int i = 0; i < column_num; ++i) {
    if (i == 0) {
      column_index[i] = pax_segments[i].len * page_header_->record_capacity;
    } else {
      column_index[i] = pax_segments[i].len * page_header_->record_capacity + column_index[i - 1];
    }
  }
...
RC PaxRecordPageHandler::insert_chunk(const Chunk &chunk, int start_row, int &insert_rows)
{
  ...
  for (int row_idx = 0; row_idx < rows_to_insert; ++row_idx) {
    ...
    for (int segment_id = 0; segment_id < page_header_->column_num; ++segment_id) {
      char *segment_data = get_field_data(slot_num, segment_id);
      memset(segment_data, 0, get_field_len(segment_id));
    }
...
RC PaxRecordPageHandler::get_chunk(Chunk &chunk)
{
  ...
  for (int chunk_col_idx = 0; chunk_col_idx < chunk.column_num(); ++chunk_col_idx) {
    int col_id = segment_id_for_field_id(chunk.column_ids(chunk_col_idx));
    if (col_id < 0 || col_id >= page_header_->column_num) {
      LOG_WARN("invalid PAX column id. field_id=%d segment_id=%d column_num=%d",
               chunk.column_ids(chunk_col_idx), col_id, page_header_->column_num);
      return RC::INVALID_ARGUMENT;
    }
...
int PaxRecordPageHandler::segment_id_for_field_id(int field_id) const
{
  ...
  for (int i = 0; i < static_cast<int>(segments.size()); ++i) {
    if (segments[i].visible && segments[i].field_id == field_id) {
      return i;
    }
  }
  return field_id;
...
RC RecordFileHandler::init_free_pages()
{
  ...
  while (bp_iterator.has_next()) {
    ...
    rc = record_page_handler->init(
        *disk_buffer_pool_, *log_handler_, current_page_num, ReadWriteMode::READ_ONLY, lob_handler_, table_meta_);
    if (rc != RC::SUCCESS) {
...
RC RecordFileHandler::insert_record(const char *data, int record_size, RID *rid)
{
  ...
  while (!free_pages_.empty()) {
    ...
    ret = record_page_handler->init(
        *disk_buffer_pool_, *log_handler_, current_page_num, ReadWriteMode::READ_WRITE, lob_handler_, table_meta_);
    if (OB_FAIL(ret)) {
      lock_.unlock();
...
RC RecordFileHandler::delete_record(const RID *rid)
{
  ...
  rc = record_page_handler->init(
      *disk_buffer_pool_, *log_handler_, rid->page_num, ReadWriteMode::READ_WRITE, lob_handler_, table_meta_);
  if (OB_FAIL(rc)) {
...
RC RecordFileHandler::get_record(const RID &rid, Record &record)
{
  ...
  // 仅为读取记录内容，使用只读模式避免在持有读锁时再尝试写锁导致的锁冲突
  RC rc = page_handler->init(
      *disk_buffer_pool_, *log_handler_, rid.page_num, ReadWriteMode::READ_ONLY, lob_handler_, table_meta_);
  if (OB_FAIL(rc)) {
...
RC RecordFileHandler::update_record(const RID &rid, const char *data)
{
  ...
  if (OB_FAIL(rc)) {
    LOG_WARN("failed to update record. rid=%s, rc=%s", rid.to_string().c_str(), strrc(rc));
    return rc;
  }
...
RC ChunkFileScanner::next_chunk(Chunk &chunk)
{
  ...
  while (bp_iterator_.has_next()) {
    ...
    rc = record_page_handler_->init(
        *disk_buffer_pool_, *log_handler_, page_num, rw_mode_, table_->lob_handler(), &table_->table_meta());
    if (OB_FAIL(rc)) {
      LOG_WARN("failed to init record page handler. page_num=%d, rc=%s", page_num, strrc(rc));
      return rc;
    }
